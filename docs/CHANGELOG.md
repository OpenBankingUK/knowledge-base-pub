...
- New FAQ clarifying that bulk reconfirmation where Deputyship/Power of Attorney arrangements are in place
...
- [What error code should be used if a TPP repeats an Idempotency ID within 24 hours?](./standards/read-write.html#what-error-code-should-be-used-if-a-tpp-repeats-an-idempotency-id-within-24-hours) - Updated answer with details of expected behaviour and new HTTP code introduced in v4.0.1
...
- Fixed some minor formatting issues (marking **must** & **should** in bold, removing superfluous `**` from some questions) to improve readability.
