...
- [Is the Data.DebtorAccount block to be provided by the ASPSP in the response block optional?](.standards/general/vrp.html#is-the-datadebtoraccount-block-to-be-provided-by-the-aspsp-in-the-response-block-optional) =- amended response for additional clarity
- [Can an ASPSP ask the PSU to re-authenticate if the trusted beneficiary was removed and need to be added back to the PSU’s trusted list?](.standards/general/vrp.html#can-an-aspsp-ask-the-psu-to-re-authenticate-if-the-trusted-beneficiary-was-removed-and-need-to-be-added-back-to-the-psus-trusted-list). Added sentence to clarify usage of Event Notifications.
